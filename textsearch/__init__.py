...
class TSResult(object):
    __slots__ = ('match', 'norm', 'start', 'end', 'case', 'is_exact', 'extras')

    def __init__(self, match, norm, start, end, case=None, exact=None, **extras):
        self.match = match
        self.norm = norm
        self.start = start
        self.end = end
...
class TextSearch(object):
    def __init__(
        ...
        Attributes:
            ...
            returns: one of 'match', 'norm', 'match_case', 'norm_case', 'TSResult' (becomes TSResult) or class
                ...
                - TSResult (str): convenient object for working with matches, e.g.:
                  TSResult(match='HI', norm='greeting', start=0, end=2, case='upper', is_exact=True, extras={})
                - class: bring your own class that will get instantiated like:
                  MyClass(**{"match": k, "norm": v, "case": "lower", "exact": False, start: 0, end: 1, **extras})
            left_bound_chars (set(str)): Characters that will determine if the word is continuing.
        ...
        Examples:
            ...
            []
            >>> ts.extract("HI")
            [TSResult(match='HI', norm='HI', start=0, end=2, case='upper', is_exact=True, extras={})]
        ...
        self.left_bound_chars = left_bound_chars
        self.right_bound_chars = right_bound_chars
    ...
    def add(self, k, v=None, **extras):
        if isinstance(k, list):
            for x in k:
                self.add_one(x, v, **extras)
        elif isinstance(k, dict):
            for k, v in k.items():
                self.add_one(k, v, **extras)
        else:
            self.add_one(k, v, **extras)
    ...
    def add_insensitive(self, k, v, length, extras):
        if self.returns == "norm":
            ...
            self.automaton.add_word(k.lower(), (length, None))
        ...
        # adding object
            ...
            self.automaton.add_word(k.lower(), (length, {"norm": v, "exact": False, **extras}))

    def add_sensitive_string(self, k, v, length, extras):
    ...
    def add_sensitive_object(self, k, v, length, extras):
        case = determine_case(k)

        obj = {"match": k, "norm": v, "case": case, "exact": True, **extras}
        ...
        for key, case in zip([k.upper(), k.title(), k.lower()], ["upper", "title", "lower"]):
            if key == k or key in self.automaton:
                continue
            obj = {"match": key, "norm": v, "case": case, "exact": False, **extras}
            self.automaton.add_word(key, (length, obj))

    def add_sensitive(self, k, v, length, extras):

        if self.returns in ["match", "match_case", "norm", "norm_case"]:
            self.add_sensitive_string(k, v, length, extras)

        else:  # going to pass some parameters to object
            self.add_sensitive_object(k, v, length, extras)

    def add_one(self, k, v=None, **extras):
        ...
        if self.case == "ignore":
            ...
            self.add_insensitive(k, v, length, extras)
        else:
            self.add_sensitive(k, v, length, extras)
    ...
    def tokenized_extract(self, text):
        if self.words_changed:
            self.add_tokenization()
        ...
        for end_index, (length, result) in self.automaton.iter(_text):
            ...
            if start >= current_stop:
                current_stop = stop
                keywords.append((stop - start, result))
            elif stop - start > keywords[-1][0]:
                current_stop = max(current_stop, stop)
                keywords[-1] = (stop - start, result)
    ...
    def extract_str(self, start_index, length, result, text):
        return result
    ...
    def __contains__(self, key):
        if self._ignore_case_in_search:
